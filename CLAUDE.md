...
`npm run cursors -- sprite [--force]` - рендерит каждый курсор манифеста в `public/win-55-ui/cursors/<cursorId>/{normal,invert}.gif` (по одной директории на физический файл, не на схему/роль - дубликаты между схемами рендерятся один раз), 2x масштаб (правило проекта для спрайтов), nearest-neighbor. `normal.gif` - обычный alpha-слой (opaque там, где AND=0, либо где реальная альфа для `windows-default`), `invert.gif` - только инверт-пиксели, перекрашенные в сплошной белый, для наложения с `mix-blend-mode: difference` поверх курсора в рантайме. Слой без единого непрозрачного пикселя не пишется (например у `crosshair`/`text` обычно нет `normal.gif` - весь рисунок сделан инвертом). Для `.ani` учитываются `seq `/`rate` чанки (реальный порядок и тайминг кадров, не просто список хранимых кадров - шаги, повторяющие один и тот же кадр подряд, схлопываются в один GIF-кадр с суммарной задержкой). Существующий файл не перезаписывается без `--force` - спрайты предполагаются доступными для ручной подрисовки в редакторе, как `.bdf` у шрифтов; удалять устаревший файл, которого больше не должно быть (курсор сменил слой), тоже руками.
...
Регистрация спрайтов в рантайме UI-кита (аналог `update-register` у шрифтов) пока не реализована - `sprite` только пишет файлы в `public/`, само использование в компонентах кита - следующий шаг.
