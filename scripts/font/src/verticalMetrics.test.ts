...
function withTempFontName(run: (fontName: string) => void): void {
  ...
  try {
    ...
    const path = resolve(srcFontDir, `${fontName}-12.measurements.json`)
...
test('resolveVerticalMetrics caches the first measurement it sees', () => {
  withTempFontName((fontName) => {
    const result = resolveVerticalMetrics(fontName, 12, { fontAscent: 10, fontDescent: 2 })
    assert.deepEqual(result.metrics, { fontAscent: 10, fontDescent: 2 })
    assert.deepEqual(result.overflow, [])
    assert.deepEqual(readCachedVerticalMetrics(fontName, 12), { fontAscent: 10, fontDescent: 2 })
  })
})

test('resolveVerticalMetrics reuses the cached value instead of a later, different measurement', () => {
  withTempFontName((fontName) => {
    resolveVerticalMetrics(fontName, 12, { fontAscent: 10, fontDescent: 2 })
    const second = resolveVerticalMetrics(fontName, 12, { fontAscent: 9, fontDescent: 3 })
...
test('resolveVerticalMetrics flags, but does not silently swallow, a later style needing more room than cached', () => {
  withTempFontName((fontName) => {
    resolveVerticalMetrics(fontName, 12, { fontAscent: 10, fontDescent: 2 })
    const bold = resolveVerticalMetrics(fontName, 12, { fontAscent: 9, fontDescent: 3 })
...
test('resolveVerticalMetrics flags ascent overflow independently from descent overflow', () => {
  withTempFontName((fontName) => {
    resolveVerticalMetrics(fontName, 12, { fontAscent: 9, fontDescent: 3 })
    const result = resolveVerticalMetrics(fontName, 12, { fontAscent: 10, fontDescent: 2 })
...
test('resolveVerticalMetrics keys the cache by (fontName, pixelSize) independently', () => {
  withTempFontName((fontName) => {
    resolveVerticalMetrics(fontName, 12, { fontAscent: 10, fontDescent: 2 })
    assert.equal(readCachedVerticalMetrics(fontName, 24), null, 'a different pixel size must not see the 12px cache')
    assert.equal(readCachedVerticalMetrics(`${fontName}-other`, 12), null, 'a different font name must not see this cache')
  })
})
