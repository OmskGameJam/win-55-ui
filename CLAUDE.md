...
**`src-cursors/manifest.json`** - `{ [schemeSlug]: SchemeEntry }`, полностью перегенерируется `discover`, руками не редактируется:
- `SchemeEntry` = `{ displayName, roles: { [role]: RoleEntry } }`. `displayName` - человекочитаемое имя схемы (из реестрового скрейпа); для новой директории без предыдущей записи - грубый тайтлкейс от slug.
- `RoleEntry`: `file` (имя файла в директории схемы), `sourceFile?` (оригинальное имя в `C:\Windows\Cursors`, либо `live:OCR_<id>` для живого захвата - провенанс, не выводится из самого файла), `reconstructed?` (см. ниже), `animated`, `width`/`height`/`hotspotX`/`hotspotY`, `bitcounts` (XOR-глубины среди кадров, обычно одна), `frameCount`, `usesInvert`/`invertFrames` (см. ниже), `suspiciousInvertFrames` (инверт-пиксели с XOR-цветом отличным от чистого белого - см. ниже).
- Всё, кроме `displayName` и `sourceFile`, `discover` при каждом запуске выводит заново из самого файла, а не доверяет старому манифесту.

`usesInvert`/`invertFrames` - обнаружение классического AND/XOR screen-invert трюка (пиксель с AND=1 и ненулевым XOR инвертирует то, что под курсором, а не рисует сплошной цвет) в каждой роли/кадре - его нельзя честно передать плоским CSS `cursor`/PNG, нужен отдельный слой с `mix-blend-mode: difference` (см. `sprite` ниже). `suspiciousInvertFrames` - инверт-пиксель с XOR-цветом не белым (а не чёрным/белым/прозрачным) - `mix-blend-mode: difference` воспроизводит побитовый XOR только на крайних значениях 0/255, так что такой пиксель либо реальный экзотический курсор (ни один из текущих 19 наборов таким не является - проверено поштучно), либо мусор от кривой конвертации; не считать намерением автоматически, смотреть глазами. `reconstructed: true` на роли помечает файлы, пересобранные из живого `HCURSOR` (все 14 ролей `windows-default`) - у них AND/XOR-структура уже сплющена в 32bpp цвет+альфа при захвате, так что `usesInvert`/`bitcounts` для них `discover` не выводит заново из файла, а сохраняет как есть из предыдущего манифеста (единственное исключение из принципа «всё пересчитывается»).

`npm run cursors -- sprite [--force]` - рендерит каждую роль манифеста в `public/win-55-ui/cursors/<scheme>/<role>/{normal,invert}.gif`, 2x масштаб (правило проекта для спрайтов), nearest-neighbor. `normal.gif` - обычный alpha-слой (opaque там, где AND=0, либо где реальная альфа для `windows-default`), `invert.gif` - только инверт-пиксели, перекрашенные в сплошной белый, для наложения с `mix-blend-mode: difference` поверх курсора в рантайме. Слой без единого непрозрачного пикселя не пишется (например у `crosshair`/`text` обычно нет `normal.gif` - весь рисунок сделан инвертом). Для `.ani` учитываются `seq `/`rate` чанки (реальный порядок и тайминг кадров, не просто список хранимых кадров - шаги, повторяющие один и тот же кадр подряд, схлопываются в один GIF-кадр с суммарной задержкой). Существующий файл не перезаписывается без `--force` - спрайты предполагаются доступными для ручной подрисовки в редакторе, как `.bdf` у шрифтов; удалять устаревший файл, которого больше не должно быть (роль сменила слой), тоже руками.

Для ролей с `reconstructed && usesInvert` (`windows-default/crosshair`, `windows-default/text`) весь `normal`-слой целиком переносится в `invert` (перекрашенный в белый), а не рендерится плоским чёрным - реконструкция стирает AND/XOR-разметку, но обе эти роли, как и их аналоги во всех остальных 18 наборах, являются «только инверт» дизайном без единого обычного непрозрачного пикселя, так что деление однозначно. Для гипотетической будущей reconstructed-роли, смешивающей инверт с настоящей заливкой, это правило было бы неверным - среди текущих 228 ролей такой нет.
