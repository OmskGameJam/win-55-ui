...
- `npm run cursors -- <command>` - toolkit для `src-cursors/` (`discover` пока единственная команда), см. «Курсоры» ниже
...
`npm run cursors -- <command>` - TS-инструмент, той же формы, что `scripts/font/` (собственный `scripts/cursors/tsconfig.json`, компилируется через `npm run cursors:build` перед запуском). Пока единственная команда - `discover`: сканирует каждую директорию под `src-cursors/`, парсит каждый `.cur`/`.ani` по его реальной бинарной структуре (`ICONDIR`/`ICONDIRENTRY`, `BITMAPINFOHEADER`, AND/XOR-маски; для `.ani` - RIFF-чанки `anih`/`fram`/`icon`, каждый `icon`-чанк - это вложенный `.cur`-blob) и полностью перезаписывает `src-cursors/manifest.json`. Метаданные роли (`width`/`height`/`hotspotX`/`hotspotY`/`bitcounts`/`animated`/`frameCount`) всегда выводятся заново из файла, а не берутся из старого манифеста - `displayName` схемы и `sourceFile` роли (оригинальное имя файла в `C:\Windows\Cursors`, либо `live:OCR_<id>` для живого захвата) переносятся из предыдущего манифеста, т.к. по самому файлу их не восстановить.

`usesInvert`/`invertFrames` - обнаружение классического AND/XOR screen-invert трюка (пиксель с AND=1 и ненулевым XOR инвертирует то, что под курсором, а не рисует сплошной цвет) в каждой роли/кадре - его нельзя честно передать плоским CSS `cursor`/PNG, это будет нужно для конвертации в веб-ассеты на следующем шаге. `reconstructed: true` на роли помечает файлы, пересобранные из живого `HCURSOR` (все 14 ролей `windows-default`) - у них AND/XOR-структура уже сплющена в 32bpp цвет+альфа при захвате, так что `usesInvert`/`bitcounts` для них `discover` не выводит заново из файла, а сохраняет как есть из предыдущего манифеста (единственное исключение из принципа «всё пересчитывается»).

Это первый шаг из более крупного планируемого тулкита (по аналогии с шрифтовым pipeline) - шаги конвертации `src-cursors/` во что-то, что реально рендерится в браузере (веб-курсоры не поддерживают AND/XOR-инверсию и `.ani`), пока не реализованы.
