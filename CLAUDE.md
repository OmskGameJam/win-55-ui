...
Одиночный пайплайн (по одному шрифту за раз): `strike` (растеризация исходного TTF в `.bdf` для конкретного pixel size; опционально авто-кернинг, см. ниже) -> опционально `fallback` + `merge` (добор недостающих глифов из другого, более полного TTF) -> ручной просмотр/правка `.bdf` (простой текстовый формат - редактировать можно прямо в нём, отдельного preview/PNG-шага нет) -> `build` (BDF -> outline TTF; `unitsPerEm = pixelSize * 100`, поэтому каждая пиксельная граница кратна 100 юнитам "по построению" - не нужно вручную сверять em-size, как в старом FontForge-процессе; мягкие находки самопроверки пишутся в `<out>.report.json`, не блокируя сборку). `strike`/`fallback`/`all` отказываются перезаписывать существующий `.bdf` без `--force`, т.к. он может быть вручную отредактирован. `npm run font -- check <bdf>` - быстрая структурная проверка (битые/дублирующиеся глифы) без полной сборки.

Манифест-driven пайплайн (по всем шрифтам сразу, источник истины - `src-font/fonts.json`): `strike-all`/`fallback-all`/`merge-all`/`build-all` повторяют те же шаги для каждой записи манифеста, у которой ещё нет соответствующего выходного файла - существующий файл не перезаписывается, а пропускается с предупреждением (можно свободно перегенерировать только недостающие части набора). `push-fonts` копирует готовые TTF из `src-font/` в `public/win-55-ui/font/` (всегда перезаписывает - это финальный шаг публикации). `update-register` полностью перегенерирует `src/helpers/generatedFonts.ts` (`SUPPORTED_FACES`) и `src/generated-fonts.css` (`@font-face`-блоки) из `fonts.json` - оба файла помечены как auto-generated, руками не редактируются. Полная схема `fonts.json` и авто-кернинг (`--generate-kerning` и связанные флаги `strike`/`fallback`) - см. `FONTS.md`.
